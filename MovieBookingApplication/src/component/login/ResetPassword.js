...
function ResetPassword(){
    ...
    const fetchUser = async() => {
        try{
            const response = await axios.get('https://moviebooking-y6k3w4hbra-uc.a.run.app/api/v1.0/moviebooking/users');
    ...
    const PasswordReset = async(event) => {
        ...
        if(user.includes(loginId)){
            if(password === confirmPassword){
                try{
                    const response = axios.put(`https://moviebooking-y6k3w4hbra-uc.a.run.app/api/v1.0/moviebooking/${loginId}/forgot`, {
                       loginId:loginId,
                       password:password
                   },{
                       headers:{
