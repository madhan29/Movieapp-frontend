...
function BookingPage(){
    ...
    const handleSubmit = (event) => {
        ...
        if(noOfTickets === seatNumbers.length){
            ...
            axios.post(`http://35.224.90.56:8085/api/v1.0/moviebooking/${movieName}/add`,{
    ...
    const fetchBookedTickets = async() => {
        try{
            const response = await axios.get(`http://35.224.90.56:8085/api/v1.0/moviebooking/getallbookedtickets/${movieName}`,{
                        headers:{
                            'Content-Type': 'application/json',
                            Authorization:`Bearer ${localStorage.getItem('accessToken')}`
                           }
